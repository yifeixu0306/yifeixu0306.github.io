...
### Chart C: Distributions of Order Interval in the Top 10 Popular Products

```{r, message=FALSE}
# Top 10 popular product
product_10 = instacart_tidy %>%
  group_by(product_name) %>% 
  summarise(count = n()) %>% 
  arrange(-count) %>% 
  top_n(10) 

product_name_10 = product_10 %>%
  pull(product_name)

instacart_tidy %>% 
  filter(product_name %in% product_name_10) %>%
  left_join(product_10, by = "product_name") %>%
  mutate(product_name = fct_reorder(product_name, -count)) %>% 
  plot_ly(x = ~product_name, y = ~days_since_prior_order, type = "box",
          color = ~product_name, alpha = 0.6) %>% 
